...
Add CSV output: timestamp\_ms,temp\_c,press\_pa.
...
Main loop consumes flag; decouple sensor timing from HAL\_Delay.
...
&nbsp;│    ├─ app\_main.c      # high-level state machine

&nbsp;│    ├─ app\_modes.c     # streaming, threshold, low-power

&nbsp;│    └─ app\_cli.c       # simple serial commands

&nbsp;├─ CoreServices/

&nbsp;│    ├─ scheduler.c     # timer tick, tasks

&nbsp;│    ├─ error\_mgr.c     # error codes -> LED patterns/logs

&nbsp;│    └─ config\_store.c  # (stretch) flash params
...
Timer tick -> set sample\_request flag.
...
On error (I²C fail, ID mismatch): error\_mgr triggers blink pattern + log.
...
LOW\_POWER: Sleep; wake on timer or button. Transitions triggered by button long-press or CLI commands.
...
INIT → STREAM ↔ THRESHOLD ↔ LOW\_POWER; button/CLI events.
...
&nbsp;├─ fig\_state\_machine.svg

&nbsp;├─ fig\_sequence\_sampling.svg
...
Use uint32\_t etc (fixed types), not bare int.



Centralized error\_t enum; map to LED blink codes.
...
&nbsp;   float temperature\_c;

&nbsp;   float pressure\_pa;

&nbsp;   uint8\_t ok;          // 1=valid, 0=error

} bmp280\_data\_t;



HAL\_StatusTypeDef BMP280\_Init(I2C\_HandleTypeDef \*hi2c);

HAL\_StatusTypeDef BMP280\_ReadData(I2C\_HandleTypeDef \*hi2c, bmp280\_data\_t \*out);
...
\#define BMP280\_ADDR  (0x76 << 1)  // adjust if your board uses 0x77
...
static int32\_t t\_fine;



static HAL\_StatusTypeDef bmp280\_read\_calib(...);

static int32\_t bmp280\_comp\_temp(...);

static uint32\_t bmp280\_comp\_press(...);
